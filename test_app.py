  ...
  # Test Endpoints for Executive Director
    ...
    def test_post_actor_executive_director(self):
        res = self.client().post(
            '/actors',
            headers=self.executive_director_bearer_token,
            json=self.new_actor)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
    ...
    def test_get_index_casting_assistant(self):
        res = self.client().get('/')
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['Author'], 'Tony Cookey')

    def test_405_get_index_casting_assistant(self):
        res = self.client().post('/')
        data = json.loads(res.data)
    ...
    def test_401_post_actor_casting_assistant(self):
        res = self.client().post(
            '/actors',
            headers=self.casting_assistant_bearer_token,
            json=self.new_actor)
        data = json.loads(res.data)
    ...
    def test_post_actor_casting_director(self):
        res = self.client().post(
            '/actors',
            headers=self.casting_director_bearer_token,
            json=self.new_actor)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_delete_actor_casting_director(self):
        res = self.client().delete('/actors/3', headers=self.casting_director_bearer_token)
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['success'], True)
        self.assertTrue(data['deleted'])
